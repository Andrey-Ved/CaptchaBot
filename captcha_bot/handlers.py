...
async def handler_new_member(message):
    ...
    text = SPEECHES['greeting with riddle'](name=message.new_chat_members[0].first_name,
    ...
    await deferred_verification(message.chat.id, test_message.message_id, current_user)
    await bot.delete_message(message.chat.id, message.message_id)
...
async def handler_process_callback(callback_query):
    ...
    if doubtful_users[current_user].captcha == button:
        ...
        await bot.answer_callback_query(callback_query.id, SPEECHES['not right'])
